...
// Démarrage du tutoriel PC
function startTutoPC() {
    const nomJoueur = localStorage.getItem('nomJoueur') || 'Joueur';
    
    // Message de bienvenue
    const message1 = `Bienvenue ${nomJoueur} ! Mode PC activé 💻`;
    typeWriter(message1, () => {
        setTimeout(() => {
            // Deuxième message expliquant les commandes
            const message2 = "Maîtrise le clavier et la souris comme un champion ! Regarde : 👇";
            typeWriter(message2, () => {
                setTimeout(() => {
                    // Afficher la démo
